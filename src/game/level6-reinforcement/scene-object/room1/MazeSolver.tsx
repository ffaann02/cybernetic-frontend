...
const MazeSolver = ({
    ...
    return (
        <>
            ...
            {isUsingSecurityCamera && mazeStateDetail.done !== true &&
                <>
                    <a.group ref={cameraRef} position={springProps.position} rotation={springProps.rotation}>
                        {(isUsingSecurityCamera && mazeStateDetail.done === false) &&
                            <PerspectiveCamera position={[0, 0, 0]} makeDefault rotation={[degreeNumberToRadian(-35), 0, 0]} />}
                    </a.group>
                    <RigidBody
                        ref={ref}
                        lockTranslations
